...
import math
from dataset.MVTec3D_IMG import MVTec3DDataset_IMG
from dataset.MVTec3D_IMG import MVTEC_CLASSES as MVTEC_CLASSES_3D
...
class PFMT_AD(object):
    ...
    def register(self, **kwargs):
        ...
        if kwargs["size"] == 256:
            ...
            H3, W3 = 64, 64
            H4, W4 = 32, 32 
        elif kwargs["size"] == 128:
            H2, W2 = 32, 32
            H3, W3 = 16, 16
            H4, W4 = 8, 8 
        elif kwargs["size"] == 192:
            H2, W2 = 48, 48
            H3, W3 = 24, 24
            H4, W4 = 12, 12
        else:
    ...
    def cal_auc(self, score_list, score_map_list, test_y_list, test_mask_list, cal_pro=False):
        ...
        if not cal_pro:
            ...
            pro_auc_score = cal_pro_metric_new(test_mask_list, score_map_list, fpr_thresh=0.3, max_steps=1000)
...
def parse_args():
    ...
    parser.add_argument("--batch_size", type=int, default=4)  # 6 or 20 for train
    ...
    parser.add_argument("--data_root", type=str, default="D:/Dataset/mvtec_3d_anomaly_detection")
...
if __name__ == "__main__":
    ...
    save_root = f"./result_3D/{name_root}_{args.seed}/"
    ...
    for class_name in MVTEC_CLASSES_3D[:]:
        torch.cuda.empty_cache()

        trans_x = T.Compose([T.Resize((args.resize,args.resize), Image.ANTIALIAS),
                            T.ToTensor(),
                            T.Normalize(mean=[0.485, 0.456, 0.406],
                                            std=[0.229, 0.224, 0.225])])

        gt_transforms = T.Compose([
            T.Resize((args.resize, args.resize), Image.NEAREST),
            T.CenterCrop(args.resize),
            T.ToTensor()])

        trainset = MVTec3DDataset_IMG(root=os.path.join(args.data_root, class_name),
                           transform=trans_x, gt_transform=gt_transforms,
                           phase='train')

        trainloader = DataLoader(trainset, batch_size=args.batch_size, shuffle=True, pin_memory=True,
                                 num_workers=0)

        testset = MVTec3DDataset_IMG(root=os.path.join(args.data_root, class_name),
                           transform=trans_x, gt_transform=gt_transforms,
                           phase='test')

        testloader = DataLoader(testset, batch_size=args.batch_size, shuffle=False, pin_memory=True, num_workers=0)
